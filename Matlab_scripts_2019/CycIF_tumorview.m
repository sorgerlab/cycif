function CycIF_tumorview(datatable,channel,sw1)
...
if sw1 == 1         % Color/digital representation
    scatter(datatable.Xt,datatable.Yt,5,log(datatable{:,channel}),'fill');colormap(jet);colorbar;
    set(gca,'XAxisLocation','bottom','YAxisLocation','left','ydir','reverse');
    set(gca,'xticklabels',{});
    set(gca,'yticklabels',{});
    
    xlim([0 (round(max(datatable.Xt)/xticksize)+1)*xticksize]);
    ylim([0 (round(max(datatable.Yt)/yticksize)+1)*yticksize]);
...
elseif sw1 == 2     % Postive/Negative
    ...
    scatter(negcells.Xt,negcells.Yt,4,'k','fill');
    hold on;
    scatter(poscells.Xt,poscells.Yt,6,'r','fill');
...
elseif sw1 == 3                % Postive density plot
    poscells = datatable(datatable{:,channel}==1,:);
    f1=scatter(datatable.Xt,datatable.Yt,5,[0.1 0.1 0.1],'fill');
    ...
    scatter(datatable.Xt,datatable.Yt,5,datatable{:,channel},'fill');colormap(jet(size(temp1,1)));colorbar;
    set(gca,'XAxisLocation','bottom','YAxisLocation','left','ydir','reverse');
    set(gca,'xticklabels',{});
    set(gca,'yticklabels',{});
    xlim([0 (round(max(datatable.Xt)/xticksize)+1)*xticksize]);
